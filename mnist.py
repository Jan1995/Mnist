...
with tf.Session() as sess:
    ...
    #
	# model.save(sess,'logs/my_net_cnn.ckpt')
	# print('模型训练好了')

	model.restore(sess, 'logs/my_net_cnn.ckpt')

	testbatch_x, testbatch_y = mnist.test.next_batch(10000)
	y_ = sess.run(output, feed_dict={x0: testbatch_x})
	y_ = np.array(np.argmax(y_, 1))
	result = np.mean(np.equal(np.argmax(testbatch_y, 1), y_))
	print('利用训练模型得到的正确率为', result * 100, '%')